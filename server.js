...
async function deepseekCleanOcr(rawText, materialType) {
  ...
  const system = materialType === "workbook"
    ? [
        "你是小学语文练习册整理助手，按中国大陆小学语文练习册的排版习惯输出。",
        "输入是 OCR 出来的练习册文本，包含识别噪声。请输出干净、可直接编辑、排版规范的题目文本。",
        "规则：",
        ...
        "5. 题号统一格式：大题用“一、二、三、”，小题用“1. 2. 3.”，选项用“A. B. C. D.”，括号选项用全角“（  ）”。",
        "6. 不同题目之间用空行分隔；同一题被 OCR 拆成多行的，按语义合并成一段。",
        "7. 标点全部使用中文全角标点：，。！？：；、“”‘’《》（）——……。把英文逗号、句号、引号、括号、问号一律换成对应的全角中文标点。",
        "8. 引号统一使用弯引号“”和‘’，不要直引号。书名用《》。",
        "9. 拼音保留时必须带声调（如 hóng shuǐ），不要无调拼音。",
        "10. 不要解释、不要 markdown，直接输出整理后的题目文本。"
      ].join("\n")
    : [
        "你是小学语文课文整理助手，按中国大陆人教版语文课本的排版习惯输出。",
        "输入是 OCR 出来的课文文本，包含识别噪声。请输出干净、可直接编辑、排版规范的课文原文。",
        "规则：",
        ...
        "4. 课文正文不要因为书本视觉换行就另起一段——必须以句号、问号、感叹号、省略号结束且语义结束才另起一段；同一自然段被 OCR 拆成多行的，按语义合并成一段。",
        "5. 每个自然段开头必须缩进两个全角空格“　　”（U+3000 表意空格，不是半角空格），课文标题独占一行不缩进。",
        "6. 自然段之间用一个空行分隔。",
        "7. 保留课文标题（去掉编号，例如“22 羿射九日”输出为“羿射九日”），标题独占一行不缩进。",
        "8. 标点全部使用中文全角标点：，。！？：；、“”‘’《》（）——……。课文中出现的英文逗号、句号、引号、括号一律换成对应的全角中文标点。",
        "9. 省略号统一为“……”（两个全角点号，共 6 个点），破折号统一为“——”（两个全角破折号），不要写成“...”或“-”。",
        "10. 引号统一使用弯引号“”和‘’，不要直引号。书名用《》。",
        "11. 句末标点和上一字之间不要有空格；中文文字内部不要混入半角空格，除非是阿拉伯数字或英文单词的分隔。",
        "12. 不要解释、不要 markdown、不要给拼音、不要给注释、不要加任何前后缀，直接输出整理后的课文。"
      ].join("\n");
...
async function deepseekPractice(payload) {
  ...
  const priorityWords = Array.isArray(payload.priorityWords)
    ? payload.priorityWords.filter(item => typeof item === "string" && item.trim()).map(item => item.trim())
    : [];
  const priorityChars = Array.isArray(payload.priorityChars)
    ? payload.priorityChars.filter(item => typeof item === "string" && item.trim()).map(item => item.trim())
    : [];
  ...
  const system = [
    ...
    "6. 按词在课文中第一次出现的顺序排列。",
    "6.1 段落覆盖（重要！）：先把课文按自然段切分，统计段数 N。挖空必须均匀分布到全文：",
    "    - 每个自然段至少要选 1 个挖空，禁止某段一个挖空都没有；除非该段不足 10 个汉字、纯标点对话短句、或全段都是助词代词等无可选实词。",
    "    - 总数按段长比例分配：长段多挑、短段少挑。先做'每段保底 1 个'的覆盖，再用剩余配额按段长加权补齐。",
    "    - 全文最后 1/3 的段落必须有挖空，禁止结尾整段空白。如果你已经选满了上限名额，就把开头/中间段落里相对次要的词替换掉，匀给末尾段落。",
    "    - 在难度允许的总数范围内，宁可在每段挑 1-2 个简单实词覆盖，也不要把所有名额都堆在前几段。",
    "7. 如果用户提供了 priorityWords（薄弱字词）列表：先扫描课文原文，把列表里在课文里出现过的词全部纳入 blanks（即使它不是你原本最想挑的高频词），再按上面规则补足到目标数量。priorityWords 里没有出现在课文中的词请直接忽略，不要硬塞。",
    "8. 如果用户提供了 priorityChars（薄弱单字）列表：在挑选 blanks 时，优先选包含这些字的词（在 priorityWords 已覆盖之外再补充），把这些字所在的词重点纳入。priorityChars 里没有出现在课文中的字请忽略，不要硬塞，也不要单独挑只有 1 个字的'词'凑数。"
  ].join("\n");
  ...
  const user = [
    ...
    `priorityWords（当前小朋友的薄弱字词，跨课文累积）：${priorityWords.length ? priorityWords.join("、") : "（无）"}`,
    `priorityChars（当前小朋友的薄弱单字，跨课文累积）：${priorityChars.length ? priorityChars.join("、") : "（无）"}`,
    "课文原文：",
    lessonText
  ].join("\n");
